...
$htmlContent = file_get_contents("http://www.mycorporateinfo.com/business/$page_data");
...
$Header = $DOM->getElementsByTagName('<body>');
...
function Gh_insert_industry_in_db($CIN,$Company_Name,$Class,$Status)
{
    ...
    if ($conn != 'falsh') {
        ...
        if ($data[0] >= 1) {
           ...
           $sql_insert = "INSERT INTO `industry_list2` (`cin`, `company_name`, `Class`, `Status`) VALUES ('".$CIN."', '".$Company_Name."', '".$Class."', '".$Status."')";
...
if($result = mysqli_query($link, $sql)){
	...
	$name_comp1 =  strtolower($name_comp);
                ...
                echo "<td>" . $row['id'] . "</td>";
                echo "<td>" . $row['cin'] . "</td>";
                echo "<td><a href='".$name_comp1."/?page=$name_comp1"."'>" . $row['company_name'] . "</a></td>";
